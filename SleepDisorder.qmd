...
trainData <- data_GBM[trainIndex, ]
testData <- data_GBM[-trainIndex, ]
...
predictions <- predict(gbm_tuned, newdata = testData)

# Evaluate the model performance
rmse <- sqrt(mean((testData$`Quality.of.Sleep` - predictions)^2))
r_squared <- 1 - sum((testData$`Quality.of.Sleep` - predictions)^2) / sum((testData$`Quality.of.Sleep` - mean(testData$`Quality.of.Sleep`))^2)
